...
SHOW_DATETIME=$(tmux show-option -gv @crimson-tmux_show_datetime 2>/dev/null)
...
date_format=$(tmux show-option -gv @crimson-tmux_date_format 2>/dev/null)
time_format=$(tmux show-option -gv @crimson-tmux_time_format 2>/dev/null)
...
echo "$RESET#[fg=${THEME[foreground]},bg=${THEME[bblack]}]$(date +"$date_string") ${separator}$(date +"$time_string")"
